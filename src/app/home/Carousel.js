...
export default function Carousel({children: slides}){
    ...
    return(
     ...
     <div className="absolute flex top-0 h-full w-full justify-between items-center text-white px-8 text-xs lg:text-2xl lg:bottom-0 ">
     ...
     <div className="relative right-0 left-0">
        <div className="flex items-center justify-center gap-2 py-6">
            {slides.map((_, i)=> (
                <div className={`transition-all w-3 h-3 bg-green-600 rounded-full ${curr ===i? "text-2xl": "bg-opacity-50 bg-slate-500"}`}></div>
            ))}
        </div>
